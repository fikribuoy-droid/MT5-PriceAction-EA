...
{
   // Try to use FOK (Fill Or Kill) first
   long filling_flags = 0;
   if(SymbolInfoInteger(symbol, SYMBOL_FILLING_FOK, filling_flags))
   {
      if(filling_flags != 0) return ORDER_FILLING_FOK;
   }
   
   // Fall back to IOC (Immediate Or Cancel)
   if(SymbolInfoInteger(symbol, SYMBOL_FILLING_IOC, filling_flags))
   {
      if(filling_flags != 0) return ORDER_FILLING_IOC;
   }
